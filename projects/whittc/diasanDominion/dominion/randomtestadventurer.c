...
int checkCard(struct gameState *testG, int currentPlayer, int n){
    int r, i, handPos, card, original_treasureTally, test_treasureTally;
    struct gameState G;
...
/*
    ...
    */
    ...
    //totalCardsBefore = G.deckCount[currentPlayer] + G.discardCount[currentPlayer] + G.handCount[currentPlayer];

    r = refactorAdventurer(card, testG, handPos);
    ...
    if(memcmp(&G, testG, sizeof(struct gameState) == 0))
        ...
        //totalCardsAfter = testG->deckCount[currentPlayer] + testG->discardCount[currentPlayer] + testG->handCount[currentPlayer];
        ...
        //check all cards still in play
        //printf("All cards in play\n");
        //assertTrue_int(totalCardsAfter, totalCardsBefore);
